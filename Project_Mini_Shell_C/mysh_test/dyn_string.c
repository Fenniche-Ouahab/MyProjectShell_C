...
{
   s->nb_case_realloc = 16;
   s->taille = 0;
   s->parcours = 0;
   s->chaine = NULL;
   s->chaine = malloc (s->nb_case_realloc * sizeof(char));
}
...
{
   if (s->taille == s->nb_case_realloc)
   {
      s->nb_case_realloc *= 2;
      s->chaine = realloc (s->chaine, s->nb_case_realloc * sizeof (char));
   }
   s->chaine[s->taille++] = c;
}
...
{
   if (s->taille == s->nb_case_realloc)
      s->chaine = realloc (s->chaine, (s->taille+1) * sizeof(char));
   s->chaine [s->taille++] = '\0';
   return s->chaine;
}
...
/**
 * Passe tous les espaces dans le flux de parcours à partir de la position courante
